...
## DEV-20260921-01 — Jira 하위 작업 등록 결과 동기화

- 날짜: 2026-09-21
- 관련 커밋 제목 / Jira: `docs: sync verified Jira subtask registrations` / DODU-38~DODU-49.
- 변경 파일: `docs/jira/WORK_BREAKDOWN.md`, `docs/jira/work-breakdown.json`, `docs/jira/README.md`, `Decisions.md`.
- 배경: 검토 가능한 단위로 준비한 하위 작업 12개가 Jira에 실제 등록되었으므로 로컬 계획 문서의 초안 상태와 실제 키·URL·의존 관계를 일치시켜야 합니다.
- 결정: DODU-38~DODU-49를 `CREATED_VERIFIED`로 기록하고, 생성 후 재조회한 부모·담당자·상태와 차단 링크 7개를 로컬 데이터에 반영합니다.
- 근거: 분할 ID 중복 검색, Subtask 유형과 부모 연결 지원 여부, 세 담당자의 할당 가능 여부를 확인한 뒤 생성했습니다. 이후 12개 티켓의 부모·담당자·설명·라벨·`할 일` 상태와 7개 차단 관계를 Jira에서 다시 조회했습니다.
- 대안 및 선택 이유: 로컬 ID만 유지하면 팀원이 실제 티켓을 찾기 어렵고 계획과 실행 상태가 어긋납니다. 실제 Jira 키를 원본 분할 ID와 함께 보존해 계획의 추적성과 Jira 접근성을 모두 유지합니다.
- 영향·주의점: Sprint·일정·Story Point와 제품 정책 승인 상태는 지정하지 않았습니다. D01~D13은 계속 OPEN이며, 관련 제안은 승인 기록 없이 확정 정책으로 취급하지 않습니다. Notion 최신 본문과 하위 문서는 접근 오류로 확인하지 못한 상태를 명시합니다.
- 검증: JSON 구문·12개 ID와 Jira 키의 유일성·부모 매핑·변환된 선행 키·순환 부재·Markdown 링크·공백 오류를 검사합니다. 코드 변경이 없어 애플리케이션 테스트는 실행하지 않습니다.
