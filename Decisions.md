...
## DEV-20260922-01 — 공개 저장소 전환과 CodeRabbit 리뷰 준비

- 날짜: 2026-09-22
- 관련 커밋: `chore: configure CodeRabbit reviews for public repository`
- 변경 파일: `.coderabbit.yaml`, `docs/CODE_REVIEW_AUTOMATION.md`, `Decisions.md`.
- 배경·근거: 사용자가 공개 후 복제본이 남는다는 설명을 확인한 뒤 Public 전환과 CodeRabbit 적용을 명시적으로 요청했습니다.
- 결정: GitHub 저장소를 Public으로 전환하고 한국어 리뷰와 dev/main PR 대상 설정을 준비합니다. 앱 설치 범위는 dodu로 제한합니다.
- 대안: 비공개 저장소 유료 리뷰 대신 공식 공개 저장소 무료 리뷰를 사용합니다. 유료 구독은 신청하지 않습니다.
- 영향: 코드와 Git 이력이 공개됩니다. 라이선스 부여는 별도 결정이며 이 작업에서 추가하지 않습니다. OPEN D01~D13은 미결정 상태를 유지합니다.
- 검증: 13개 커밋의 주요 토큰·개인키 패턴 검사에서 일치 항목이 없었고 GitHub API에서 PUBLIC 상태를 확인했습니다. 설정은 공식 스키마와 대조하며 실제 앱 설치·리뷰 실행은 미확인입니다. 앱 코드 변경은 없습니다.
